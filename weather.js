...
let weather = {
  ...
  fetchVreme: async function (lat, long) {
    ...
    document.querySelector(".city").innerText =
      "Weather in " + searchBarInput.value; // <<-- Location in heading
    document.querySelector(".temp").innerText =
      parseInt(data.current_weather.temperature) + "°C"; // <<-- Temperature
    document.querySelector(".windspeed").innerText =
      "Wind: " + parseInt(wspeed) + "km/h"; // <<-- Wind Speed
    document.querySelector(".humidity").innerText =
      "Humidity: " + parseInt(hourlyWeather.relativehumidity_2m[0]) + "%"; // <<-- Humidity
    ...
    document.querySelector(".weatherIcon").src =
      "/Icons/" + codeDescription + ".svg"; // <<-- Weather icons
  ...
  getCoordForCity: async function (cityName) {
    ...
    const MAPBOX_ACCESS_TOKEN =
      "pk.eyJ1IjoiY3Jpc3RpYW5tYWNvdmVpIiwiYSI6ImNsZzUwNzFicDAwMzYzZHByeHd2cXp4cjEifQ.TF3o7k2iti3xgiQNOcQVFA";
  ...
  (position) => {
    ...
    fetch(apiUrl)
      ...
      .then((data) => {
        ...
        var cityComponent = addressComponents.find((component) =>
